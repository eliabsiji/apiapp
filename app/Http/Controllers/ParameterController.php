...
class ParameterController extends Controller {
    public function index() {
        ...
        $parameter = app_parameter_Model::where( 'user_id', Auth::user()->id )->leftjoin( 'users', 'app_parameter_models.user_id', '=', 'users.id' )
        ->get( [ 'users.id as id', 'app_parameter_models.parameter as parameter','app_parameter_models.id as parameterid',
        'app_parameter_models.description as description',
        'app_parameter_models.created_at as datecreated' ] );
        return view( 'stylesetting.parameter' )->with( 'parameter', $parameter );
    ...
    public function editparameter( $id ) {
        $parameter  =  app_parameter_Model::find( $id );
